...
public class GridHelper {
	...
	public static void generateGrid(SudokuGrid sudokuGrid, Difficulty difficulty) {
		...
		setStartingCells(sudokuGrid.getListGrid());
	...
	private static LinkedList<Integer> getShuffledList(int amount) {
	...
	public static SudokuGrid stringToGrid(String[] stringGrids, char zeroIndicator) {
		SudokuGrid sudokuGrid = new SudokuGrid();
		List<Cell> listGrid = sudokuGrid.getListGrid();
		if (stringGrids[1] == null) {
			stringGrids[1] = stringGrids[0];
		}

		for (int i = 0; i < FIELDSIZE * FIELDSIZE; i++) {
			if (stringGrids[0].charAt(i) == zeroIndicator && stringGrids[1].charAt(i) == zeroIndicator) {
				listGrid.add(new Cell(0, i / 9, i % 9, false));
			} else {
				if (stringGrids[0].charAt(i) != zeroIndicator) {
					Integer number = Character.getNumericValue(stringGrids[0].charAt(i));
					listGrid.add(new Cell(number, i / 9, i % 9, true));
				} else if (stringGrids[1].charAt(i) != zeroIndicator) {
					Integer number = Character.getNumericValue(stringGrids[1].charAt(i));
					listGrid.add(new Cell(number, i / 9, i % 9, false));
				}
			}
		}
		...
		for (int i = 0; i < sudokuGrid.getListGrid().size(); i++) {
			...
			if (cell.getNumber() == 0) {
				...
				if (cell.isStartingCell()) {
					initialGrid.append(cell.getNumber());
					entryGrid.append(zeroIndicator);
				} else {
					entryGrid.append(cell.getNumber());
					initialGrid.append(zeroIndicator);
				}
			}
		}
